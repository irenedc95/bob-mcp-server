...
async function communicateWithBob(request: {
  ...
  console.error(
    "[Bob MCP] ISTRUZIONI: Apri il file request.json, elabora la richiesta con Bob, e salva la risposta in response.json"
  );
  ...
  throw new Error(
    "Timeout: Bob non ha risposto entro 5 minuti. Assicurati di elaborare la richiesta e salvare la risposta."
  );
}
  ...
  const tools: Tool[] = [
    {
      name: "generate_code",
      description:
        "Genera codice in base a una descrizione. Bob analizzerà il prompt e genererà codice di alta qualità nel linguaggio specificato.",
      inputSchema: {
        type: "object",
        properties: {
          prompt: {
          ...
          language: {
            type: "string",
            description:
              "Linguaggio di programmazione (es: python, javascript, typescript, java, go, rust)",
          },
          context: {
            type: "string",
            description:
              "Contesto aggiuntivo, requisiti specifici, o vincoli da considerare",
          },
        },
        required: ["prompt"],
      },
  ...
  if (request.params.name === "generate_code") {
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new Error(
          `Parametri non validi: ${error.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join(", ")}`
        );
      }
      throw error;
    }
